      ...
      <div className="relative w-full max-w-md">
        ...
        <FadeUp>
          <div className="flex flex-col items-center text-center">
            ...
            <h1 className="mt-7 font-display text-3xl font-black text-[var(--text-primary)]">Create your account</h1>
            <p className="mt-2 text-sm font-medium text-[var(--text-muted)]">Start exploring in minutes — free forever.</p>
          </div>
        </FadeUp>
        ...
        <FadeUp delay={0.08}>
          <div className="relative mt-9 rounded-[2rem] border border-[var(--border)] bg-[var(--surface-glass)] p-8 backdrop-blur-xl sm:p-10">
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-5">
              ...
              >
                {busy ? "Creating account…" : "Create free account"} {!busy && <ArrowRight size={16} />}
              </button>
            </form>

            <p className="mt-6 text-center text-sm font-semibold text-[var(--text-muted)]">
              Already have an account?{" "}
              <Link to="/login" className="font-bold text-[var(--primary)] transition hover:text-[var(--text-primary)]">
                Log in
              </Link>
            </p>
          </div>
